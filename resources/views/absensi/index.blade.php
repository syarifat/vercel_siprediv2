...
<div class="max-w-6xl mx-auto mt-8">
    ...
    <!-- Card Rekap Status (clickable) with inline details -->
...
function fetchAbsensi() {
    ...
    fetch(url)
        ...
        .then(data => {
            ...
            // Ambil semua siswa dari rombel di kelas yang difilter untuk tahun ajaran aktif
            fetch(`/api/siswa?kelas_id=${kelas_id}`)
                .then(res => res.json())
                .then(rombels => {
                    // Debug: log jumlah siswa yang diterima dari API
                    console.log('Jumlah siswa dari rombel:', rombels.length);
                    ...
                    // Gunakan data dari rombel, yang sudah difilter berdasarkan tahun ajaran
                    let countBelum = rombels.filter(rombel => !absenNis.includes(rombel.nis)).length;
                    ...
                    window.latestSiswaList = rombels; // Perbaikan: menggunakan rombels bukan siswaList
...
function renderCardDetails(status) {
    ...
    const renderRows = (rows, includeNomorAbsen = true) => {
        if (!rows.length) return `<tr><td colspan="5" class="px-4 py-2 text-center text-gray-500">Tidak ada data</td></tr>`;
        return rows.map((r, i) => `
            <tr class="bg-white border-b ${colors.border} ${colors.hover}">
                ...
                ${includeNomorAbsen ? `<td class="px-2 sm:px-4 py-1 sm:py-2 text-center">${r.nomor_absen ?? '-'}</td>` : ''}
            </tr>
        `).join('');
    };
    ...
    if (status === 'belum') {
        ...
        rowsHtml = renderRows(missing, true);
    } else {
        ...
        const filtered = absensi
            ...
            .map(a => ({ 
                ...
                nis: a.siswa_nis, 
                kelas: a.kelas_nama,
                nomor_absen: status === 'hadir' ? null : (a.nomor_absen ?? '-')
            }));
        showNomorAbsen = status !== 'hadir';
        rowsHtml = renderRows(filtered, showNomorAbsen);
    }

    container.innerHTML = `
        <div class="bg-white border rounded-lg shadow p-4">
            <div class="flex items-center justify-between mb-3">
                <h3 class="font-semibold text-lg ${colors.text}">${title} — ${tanggal} ${kelasId ? '(Kelas filter aktif)' : ''}</h3>
                ...
                <table class="min-w-full border-2 ${colors.border} rounded-lg overflow-hidden shadow table-auto text-sm">
                    <thead>
                        <tr class="${colors.bg} ${colors.text}">
                            ...
                            ${showNomorAbsen ? `<th class="px-2 sm:px-4 py-1 sm:py-2 text-center">No. Absen</th>` : ''}
                        </tr>
                    </thead>
                    <tbody>
                        ${rowsHtml}
